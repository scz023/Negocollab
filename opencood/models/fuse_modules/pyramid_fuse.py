...
def weighted_fuse(x, score, record_len, affine_matrix, align_corners, warp = True):
    ...
    warp : bool
        whether warp neb feature or not. 
        In inference setting, feature has wapprd in receiver, so do not need to warp here.
    """
    ...
    for b in range(B):
        ...
        if warp: 
            ...
            feature_in_ego = batch_node_features[b]
            scores_in_ego = split_score[b]
            
        # 在计算 softmax 前进行数值稳定性处理
        # scores_in_ego.masked_fill_(scores_in_ego == 0, -float('inf'))
        scores_in_ego.masked_fill_(scores_in_ego == 0, -1e-6)
        ...
        scores_in_ego = torch.where(torch.isnan(scores_in_ego), 
                                    ...
                                    scores_in_ego)
        # feature_show(feature_in_ego[1], 'analysis/pnpda_debug/neb_af_warp')
        out.append(torch.sum(feature_in_ego * scores_in_ego, dim=0))
    # feature_show(out[0], 'analysis/pnpda_debug/fuse')
    out = torch.stack(out)
...
class PyramidFusion(ResNetBEVBackbone):
    ...
    def forward_collab(self, spatial_features, record_len, affine_matrix, agent_modality_list = None, cam_crop_info = None, warp = True):
        """
        spatial_features : torch.tensor
            [sum(record_len), C, H, W]
        ...
        for i in range(self.num_levels):
            ...
            fused_feature_list.append(weighted_fuse(feature_list[i], score, record_len, affine_matrix, self.align_corners, warp))
