...
class HeterPnpdaPairTrain(nn.Module):
    def __init__(self, args):
        super(HeterPnpdaPairTrain, self).__init__()
        self.args = args
    ...
    def model_train_init(self):
        ...
        for name, p in self.named_parameters():
            if ('adapter_' in name ) and ('enhancer' not in name):
                p.requires_grad = True
                
        for name, module in self.named_children():
            if 'adapter_' in name:
                for name_sub_modu, sub_module in module.named_modules():
                    if 'enhancer' not in name_sub_modu:
                        sub_module.train()
    ...
    def forward(self, data_dict):
        ...
        adapted_heter_feature_2d, query_for_train, key_for_train, adapt_agent_idx  \
              = eval(f'self.adapter_{modality_name}')(heter_feature_2d, \
                                                 agent_modality_list, \
                                                 record_len, \
                                                 affine_matrix)
        ...
        if fusion_method == 'pyramid' and eval(f"self.shrink_flag_{ego_modality}"):
