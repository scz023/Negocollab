...
class HeterNegoTrain(nn.Module):
    def __init__(self, args):
        ...
        if self.allied_num == 0:
            if self.stage == 'nego': # 训练协商器阶段, 全为未在联盟内的代理时, 初始化各模态备份的协商器参数为相同值
    ...
    def forward(self, data_dict):
        ...
        if num_allied > 0: # 所有allied智能体标准特征的平均值作为公共表征
            ...
            if self.pc_grad:
                ...
                common_rep_for_mode_dict = {}
                for modality_name in self.modality_name_list:
                    common_rep_for_mode_dict[modality_name] = \
                        eval(f'self.negotiator_bak_{modality_name}')(standard_feature_list)
                common_rep = torch.mean(torch.stack(list(common_rep_for_mode_dict.values())), dim=0)
        ...
        for modality_name in self.modality_name_list:            
            if num_allied == 0:
                # if self.stage == 'nego':
                if self.pc_grad:
                    receiver_feature = common_rep_for_mode_dict[modality_name]
                else:
        ...
        #     restored_feature = output_dict["fc_after_receive"][modality_name]                    
    ...
    def unify_pragma_loss(self, common_rep, modality_feat_common_dict, record_len, affine_matrix):
        """
        ...
        if fusion_name_nego == 'pyramid':
            common_fuse, occ_outputs = self.fusion_net_nego(common_rep, record_len, affine_matrix)
        else:
            common_fuse = self.fusion_net_nego(common_rep, record_len, affine_matrix)
            occ_outputs = None
            
        cls_preds_nego = self.cls_head_nego(common_fuse)
        reg_preds_nego = self.reg_head_nego(common_fuse)
        dir_preds_nego = self.dir_head_nego(common_fuse)
        preds_nego['nego'] = {
