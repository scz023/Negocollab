...
class HeterNegoCollabInLocal(nn.Module):
    def __init__(self, args):
        super(HeterNegoCollabInLocal, self).__init__()
        self.args = args
        ...
        # if self.
        # self.pub_codebook = torch.load(args['pub']['cb_path']) # 推理状态下, 默认使用公共基向量
        # if self.pub_codebook is not None:
            # self.pub_codebook.requires_grad = False

        
        self.test_modality = args['comm_space']
        assert self.test_modality in self.modality_name_list
        
        modality_name = self.test_modality
        ...
        """
        model_setting['comm_args'].update({
                                            'local_range': self.cav_range,
                                            'fixed':self.fixed[modality_name],
                                            'comm_space': self.test_modality,
    ...
    def forward(self, data_dict):
        ...
        # feature_show(feature[0], f'analysis/local/before_send')
        
        """
        Communicate by codebook
        """
        subfeats_bf_align2 =  eval(f"self.comm_{modality_name}.sender")(feature)
        sub_feats =  eval(f"self.comm_{modality_name}.sender_align")(subfeats_bf_align2)
        feature =  eval(f"self.comm_{modality_name}.receiver")(sub_feats,  record_len, affine_matrix)
        
        
        # feature_show(feature[0], f'analysis/local/after_send')
