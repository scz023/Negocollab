...
class HeterNegoInteractTrainVanilla(nn.Module):
    def __init__(self, args):
        super(HeterNegoInteractTrainVanilla, self).__init__()
        ...
        if self.allied_num == 0: # 全为未在联盟内的代理时, 初始化各模态备份的协商器参数为相同值
    ...
    # def _negotiator_grad_update(self):
        
    #     new_para_dict = OrderedDict()
    #     """计算各negotiator梯度之和"""
    #     for modality_name in self.newtype_modality_list:
    #         for name, para in eval(f'self.negotiator_bak_{modality_name}.parameters()'):
    #             if name not in new_para_dict.keys():
    #                 new_para_dict[name] = para
    #             else:
    #                 new_para_dict[name] = new_para_dict[name] + para
        
    #     """计算参数平均值, 赋给各智能体的negotiator"""
    #     num_new_agents = len(self.newtype_modality_list)
    #     for name, para in new_para_dict.items():
    #         new_para_dict[name] = para / num_new_agents
            
    #     for modality_name in self.newtype_modality_list:
    #         for name, para in eval(f'self.negotiator_bak_{modality_name}.parameters()'):
    #             para.data.copy_(new_para_dict[name].data)
    #     def _negotiator_para_update(self):
    ...
    def forward(self, data_dict):
        ...
        for modality_name in self.modality_name_list:
            ...
            #     if self.pc_grad:
            #         modality_feature_dict[modality_name] = eval(f"self.comm_{modality_name}.receiver")\
            #             (consensus_feature_for_mode_dict[modality_name], record_len, affine_matrix)
            #     else:
            #         modality_feature_dict[modality_name] = eval(f"self.comm_{modality_name}.receiver")\
            #         (consensus_feature, record_len, affine_matrix)
