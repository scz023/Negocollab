...
def test_parser():
    ...
    parser.add_argument('--agent_type', '-type', type=str, default='general',
    ...
    parser.add_argument('--loc_err', type=int, default=0, help='localization error: loc_err * 0.1')
...
def main():
    ...
    """Wild setting"""
    # loc_err = float(opt.loc_err) 
    # print(loc_err)
    loc_err = float(opt.loc_err) * 0.1
    if loc_err > 0:
        hypes['noise_setting'] = {
            'add_noise': True,
            'args': {
    ...
    # hypes['train_params'].update({'max_cav': hypes['train_setting']['train_params']})
    # hypes['model']['args'].update({'stage': 'nego'})
    # 关于max_cav: 这里默认为5, 这是因为数据集加载max_cav时, 默认选择为train_params下的max_cav, 而这里修改之后的yaml文件的train_params
    # 放到了train_setting子目录下, 数据记载代码会判断为无train_params, 故设为默认值5

    # 测试同构协作表现, 将所有智能体模态替换为协作模态
    if  'm' in opt.agent_type:
        # hypes['fusion']['core_method'] = 'intermediateheter'
        mapping_dict = hypes['heter']['mapping_dict']
        for src in mapping_dict.keys():
            mapping_dict[src] = opt.agent_type
    ...
    if 'm' in opt.agent_type:
        resume_epoch, model = train_utils.load_modality_saved_model_ap(saved_path, model, opt.agent_type, \
            True, opt.eval_epoch)
    else:
        resume_epoch, model = train_utils.load_saved_model_ap(saved_path, model, opt.eval_epoch)
    ...
    if resume_epoch == 0: # 若无预训练的协作模型, 从保存在stage0_mx_collab的local模型加载
        ...
        resume_epoch = 'collab_w_pretrained_comm' # individual combine    
    ...
    infer_info = opt.agent_type + '_' + opt.fusion_method + opt.note
    ...
    with open(os.path.join(saved_path, result_file), 'a+') as f:
        if loc_err > 0:
            f.write('\nloc_err: {:.01f}\n'.format(loc_err))
        msg = ''
        if ('m' in opt.agent_type) or (opt.agent_type == 'hete'): 
            msg += f'{opt.agent_type}: '
