...
def main():
    ...
    """opt.stage: nego, add"""
    hypes['model']['core_method'] = hypes['model']['core_method'][f'train']
    ...
    if hypes['train_setting']['pc_grad'] and len(model.newtype_modality_list) > 0:
    ...
    for epoch in range(init_epoch, max(epoches, init_epoch)):
        ...
        if epoch % hypes['train_params']['eval_freq'] == 0 or epoch == 1:           
            ...
            if run_test:
                thread_test = threading.Thread(target=epoch_test, args=(hypes, opt, epoch))
                thread_test.start()
...
def epoch_test(hypes, opt, epoch, thread_val = None):
    ...
    for mode in modes:
        ...
        cmd = f"python opencood/tools/inference_nego_vanilla.py \
